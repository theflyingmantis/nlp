...
class StupidBackoffLanguageModel:
  ...
  def __init__(self, corpus):
    ...
    self.vocab=set([])
    self.train(corpus)
    self.lamb = 0.4 #setting labda for stupid backoff as 0.4
  ...
  def score(self, sentence):
    ...
    for token in sentence[2:]:
        ...
        if count_trigram != 0:
            ...
            score += math.log(count_bigram) #+ math.log (0.4)
            score -= math.log(self.lapunigramCounts[p2])
        else:
            score += math.log(self.lapunigramCounts[token] + 1) #+ 2*math.log (0.4)
